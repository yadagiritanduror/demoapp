class SoftwaresController < ApplicationController
  ...
  def index
    ...
    @software_count = "software has #{Software.count} records"
  end
  ...
  def ajax_demo
   (0..2).each do |i|
    Software.create(name: "name#{i}", category:"category#{i}", industry: "industry#{i}")
   end
  end
  ...
  private
    ...
    def software_params
      params.require(:software).permit(:name, :category, :industry)
    end
end
